...
void multiAssign(int multiplicand[], int multiplier[], int& multiplicandSize, int multiplierSize)
{
	int product[arraySize] = {}; //自己弄一個商出來
	...
	for (int i = 0; i < productSize; i++) { //再把商的值給被乘數
		multiplicand[i] = product[i];
	}

	for (int i = 0; i < multiplicandSize; i++) //進位
	{
	...
	if (multiplicand[multiplicandSize] == 0) multiplicandSize--; //去零

}

void division(int dividend[], int divisor[], int quotient[], int remainder[],
	int dividendSize, int divisorSize, int& quotientSize, int& remainderSize) //這邊我全部照基哥的ppt
{
	if (isZero(dividend, dividendSize)) { //如果被除數是0, 那商跟餘數都會是0
		quotient[0] = 0;
		quotientSize = 1;
		remainder[0] = 0;
		remainderSize = 1;
	...
	remainderSize = dividendSize; //複製餘數
	...
	if (less(dividend, divisor, dividendSize, divisorSize)) { //如果被除數 < 除數, 商為0, 被除數直接變成餘數
		quotient[0] = 0; //商為0
		quotientSize = 1; 
		return;
	}
	...
	int buffer[arraySize] = {}; //先把buffer建出來
	int bufferSize;
	...
	for (i = dividendSize - divisorSize; i >= 0; i--, j--) {
		...
		if (a <= b) {
			quotient[i] = b / a;//商的第i個位置放b/a
			multiplication(divisor, quotient[i], buffer, divisorSize, i, bufferSize);
			while (less(remainder, buffer, remainderSize, bufferSize)) {//如果buffer乘完比餘數大商就先-1再跑一次 範例在紙上
				quotient[i]--;
				multiplication(divisor, quotient[i], buffer, divisorSize, i, bufferSize);
...
void multiplication(int multiplicand[], int multiplier, int product[],
	int multiplicandSize, int multiplierPos, int& productSize)  //這邊要做的事情是 product = multiplicand * multiplier * ( 10 ^ multiplierPos )不是乘法
{
	...
	productSize = multiplicandSize + multiplierPos + 2;//這邊我會加二是隨興加的 因為我知道傳進來的multiplier不會超過10
	for (int i = 0; i < productSize; i++) {//先把結果會用到的空間全部歸零
		product[i] = 0;
	}

	for (int i = 0; i < multiplicandSize; i++) {//整個被乘數跑過一輪
		product[i + multiplierPos] = multiplicand[i] * multiplier; //結果出來之後還要在右邊補multiplierPos個 '0', 所以我這邊直接把結果放在 [i+multiplierPos]裡面
	}
	...
	for (int i = productSize - 1; i > 0; i--) { //去零頭
		if (product[i] == 0) {
			productSize--;
		}
		else {
